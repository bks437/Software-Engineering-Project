<?php
	...
	if(strcmp($_GET[action],"Taught")==0){
		...
		$result = pg_execute($dbconn,"Taught",array($_GET[username],$_GET[course]));
	}
	elseif(strcmp($_GET[action],"Teaching")==0){
		pg_prepare($dbconn,"Teaching",'INSERT INTO DDL.are_teaching VALUES ($1,$2)');
		$result = pg_execute($dbconn,"Teaching",array($_GET[username],$_GET[course]));
	}
	elseif(strcmp($_GET[action],"Wants")==0){
		pg_prepare($dbconn,"Wants",'INSERT INTO DDL.wants_to_teach VALUES ($1,$2,$3)');
		$result = pg_execute($dbconn,"Wants",array($_GET[username], $_GET[grade],$_GET[course]));
	}
	if(!$result)
		echo json_encode("Failed to add class.");
	else
